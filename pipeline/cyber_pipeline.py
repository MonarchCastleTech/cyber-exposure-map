# -*- coding: utf-8 -*-
"""Cyber Threat Exposure Map - Data Extraction Pipeline"""
import os
import json
import yaml
import requests
...
def generate_demo_data(config):
    ...
    return {
        "meta": {
            ...
            "sources": ["CISA KEV", "AlienVault OTX", "Shodan", "MalwareBazaar"],
            "version": "1.0.0"
        },
        "stats": [
            {"label": "Active Exploits", "value": "347", "delta": "+5%"},
            {"label": "Exposed Assets", "value": "12.4K", "delta": "+2"},
            {"label": "Threat Actors", "value": "89", "delta": "-3%"},
            {"label": "CVEs (30d)", "value": "1,203", "delta": "stable"},
        ],
        "entities": [
        ...
        "events": [
            {"id": i, "title": f"Event {i}", "severity": ["low", "medium", "high"][i % 3], "timestamp": datetime.now(timezone.utc).isoformat(), "source": "CISA KEV"}
            for i in range(1, 16)
        ],
        "timeseries": [
