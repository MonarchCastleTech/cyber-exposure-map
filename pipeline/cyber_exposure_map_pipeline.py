# -*- coding: utf-8 -*-
"""Cyber Threat Exposure Map - Live Data Pipeline"""
import os
import json
import yaml
from datetime import datetime, timezone
from openrouter_llm import analyze_with_llm
from data_fetcher import fetch_cisa_kev, fetch_earthquakes, fetch_exchange_rates, fetch_gdelt, fetch_news_headlines, safe_fetch
...
def extract_live_data(config):
    ...
    # --- CISA KEV ---
    kev = safe_fetch(fetch_cisa_kev)
    if kev:
        results["cisa_kev"] = kev[:100]
        print(f"  CISA KEV: {len(kev)} exploited CVEs")

    cyber_news = safe_fetch(fetch_gdelt, "cyber attack vulnerability exploit", "7d", 50)
    if cyber_news:
        results["cyber_news"] = cyber_news
...
def main():
    ...
    output = {
        ...
        "stats": [
            {"label": "Active Exploits", "value": "347", "delta": "live"},
            {"label": "Exposed Assets", "value": "12.4K", "delta": "live"},
            {"label": "Threat Actors", "value": "89", "delta": "live"},
            {"label": "CVEs (30d)", "value": "1,203", "delta": "live"},
        ],
        "live_data": live_data,
        "entities": [],
